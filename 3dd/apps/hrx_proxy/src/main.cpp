...
/**
 * @brief Relays network traffic between the guard and htx_proxy
 *
...
int main(int argc, char *argv[]) {
    if (argc < 4) {
        std::cerr
            << "Usage: " << argv[0] << "\n"
            << "\t<src_port>: port where the proxy receives traffic (from the guard)\n"
            << "\t<dst_ip>: IP address where the proxy sends traffic (to htx_proxy)\n"
